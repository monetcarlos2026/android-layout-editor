...
<div class="statusbar"><div class="dot"></div><span id="statusText">就绪</span><span class="status-sep">|</span><span id="statusComponents">组件: 0</span><span class="status-sep">|</span><span id="statusDevice">Pixel 7 (xxhdpi)</span><span class="status-sep">|</span><span id="statusTheme">Light</span><span style="flex:1"></span><span>Android Layout Editor v2.13.0</span></div>
...
const text=doc.getText();const line=doc.lineAt(position).line;
...
const text=doc.getText();const edits=[];
try{const fmt=this._fmtXml(text);if(fmt!==text)edits.push(vs.TextEdit.replace(new vs.Range(0,0,doc.lineCount,0),fmt));}catch(e){}
return edits;
}
_fmtXml(text){
let out='';const lines=text.split('\n');let depth=0;
for(const line of lines){const tl=line.trim();if(!tl)continue;
if(tl.startsWith('<?xml')){out+=tl+'\n';continue;}
if(tl.startsWith('</'))depth=Math.max(0,depth-1);
const indent='    '.repeat(depth);
if(tl.startsWith('<')&&!tl.startsWith('</')&&!tl.endsWith('/>')){const tag=tl.match(/<(\w+)/);if(tag&&!LW.has(tag[1])&&!tl.includes('/>'))depth++;}
let fmt=indent+tl;
const attrs=fmt.match(/\s(\w+:)?[\w_]+="[^"]*"/g)||[];
if(attrs.length>3){const tagEnd=fmt.indexOf('>');if(tagEnd>0){const tag=fmt.substring(0,fmt.indexOf(' '));const rest=fmt.substring(fmt.indexOf(' '),tagEnd+1);const sorted=rest.split(/\s+(?=\w+:)/).sort().join('\n'+indent+'    ');fmt=tag+'\n'+indent+'    '+sorted;}}
out+=fmt+'\n';
if(tl.endsWith('/>'))depth=Math.max(0,depth-1);}
return out.trim()+'\n';
}
}
class RsrcProvider{
async provideCompletionItems(doc,position){
const line=doc.lineAt(position).text.substring(0,position.character);
const m=line.match(/@(\w+)\/$/);if(!m)return;
const type=m[1];const wf=vs.workspace.workspaceFolders;if(!wf)return;
const items=[];
const scanDir=(dir,typePrefix)=>{
try{const entries=fs.readdirSync(dir,{withFileTypes:true});
for(const ent of entries){
if(ent.isDirectory())continue;
const name=ent.name.replace(/\.xml$/,'').replace(/\.png$/,'').replace(/\.jpg$/,'').replace(/\.webp$/,'');
const item=new vs.CompletionItem(name,vs.CompletionItemKind.Reference);
item.insertText=typePrefix+name;item.detail='@'+typePrefix+name;items.push(item);
}}catch(e){}
};
if(type==='drawable')scanDir(path.join(wf[0].uri.fsPath,'res','drawable'),'drawable/');
else if(type==='layout')scanDir(path.join(wf[0].uri.fsPath,'res','layout'),'layout/');
else if(type==='string')scanDir(path.join(wf[0].uri.fsPath,'res','values'),'string/');
else if(type==='dimen')scanDir(path.join(wf[0].uri.fsPath,'res','values'),'dimen/');
return items;
}
}
class HoverProvider{
...
const m=line.match(/@(\w+)\/([a-zA-Z0-9_]+)/);if(!m)return;
const type=m[1],name=m[2];const wf=vs.workspace.workspaceFolders;if(!wf)return;
let val='';
if(type==='string'||type==='dimen'){
try{const fp=path.join(wf[0].uri.fsPath,'res','values',type+'s.xml');
const c=fs.readFileSync(fp,'utf-8');const rx=new RegExp('<item name="'+name+'">([^<]+)</item>');const mm=c.match(rx);if(mm)val=mm[1];}catch(e){}
}
const md=new vs.MarkdownString();
md.appendMarkdown('**@'+type+'/'+name+'**\n\n');
if(val)md.appendMarkdown('Value: `'+val+'`\n\n');
md.appendMarkdown('Type: `'+type+'`\n');
return new vs.Hover(md);
}
}
...
ctx.subscriptions.push(vs.languages.registerCompletionItemProvider({scheme:'file',pattern:'**/*.xml'},new RsrcProvider(),'/'));
ctx.subscriptions.push(vs.languages.registerHoverProvider({scheme:'file',pattern:'**/*.xml'},new HoverProvider()));
...
ctx.subscriptions.push(vs.commands.registerCommand('androidLayoutEditor.formatDocument',async()=>{const ed=vs.window.activeTextEditor;if(!ed||!ed.document.uri.path.endsWith('.xml')){vs.window.showWarningMessage('请先打开XML文件');return;}const prov=new FmtProvider();const edits=prov.provideDocumentFormattingEdits(ed.document);if(edits&&edits.length>0){const edit=new vs.WorkspaceEdit();edits.forEach(e=>edit.replace(ed.document.uri,e.range,e.newText));await vs.workspace.applyEdit(edit);vs.window.showInformationMessage('格式化完成');}else{vs.window.showInformationMessage('无需格式化');}}));
ctx.subscriptions.push(vs.commands.registerCommand('androidLayoutEditor.extractToString',async()=>{const ed=vs.window.activeTextEditor;if(!ed)return;const sel=ed.selection;if(sel.isEmpty){vs.window.showWarningMessage('请先选择要提取的文本');return;}const text=ed.document.getText(sel);const name=await vs.window.showInputBox({prompt:'输入字符串资源名',placeHolder:'app_name'});if(!name)return;const wf=vs.workspace.workspaceFolders;if(!wf)return;const sf=vs.Uri.joinPath(wf[0].uri,'res','values','strings.xml');let content='';try{content=new TextDecoder().decode(await vscode.workspace.fs.readFile(sf));}catch(e){content='<?xml version="1.0" encoding="utf-8"?>\n<resources>\n</resources>';}const insert='    <string name="'+name+'">'+text+'</string>\n';const closeIdx=content.lastIndexOf('</resources>');if(closeIdx>=0){const newContent=content.substring(0,closeIdx)+insert+content.substring(closeIdx);await vscode.workspace.fs.writeFile(sf,new TextEncoder().encode(newContent));}const edit=new vs.WorkspaceEdit();edit.replace(ed.document.uri,sel,'@string/'+name);await vs.workspace.applyEdit(edit);vs.window.showInformationMessage('已提取到 strings.xml: @string/'+name);}));
ctx.subscriptions.push(vs.commands.registerCommand('androidLayoutEditor.extractToDimen',async()=>{const ed=vs.window.activeTextEditor;if(!ed)return;const sel=ed.selection;if(sel.isEmpty){vs.window.showWarningMessage('请先选择尺寸值');return;}const text=ed.document.getText(sel);const name=await vs.window.showInputBox({prompt:'输入尺寸资源名',placeHolder:'margin_large'});if(!name)return;const wf=vs.workspace.workspaceFolders;if(!wf)return;const df=vs.Uri.joinPath(wf[0].uri,'res','values','dimens.xml');let content='';try{content=new TextDecoder().decode(await vscode.workspace.fs.readFile(df));}catch(e){content='<?xml version="1.0" encoding="utf-8"?>\n<resources>\n</resources>';}const insert='    <dimen name="'+name+'">'+text+'</dimen>\n';const closeIdx=content.lastIndexOf('</resources>');if(closeIdx>=0){const newContent=content.substring(0,closeIdx)+insert+content.substring(closeIdx);await vscode.workspace.fs.writeFile(df,new TextEncoder().encode(newContent));}const edit=new vs.WorkspaceEdit();edit.replace(ed.document.uri,sel,'@dimen/'+name);await vs.workspace.applyEdit(edit);vs.window.showInformationMessage('已提取到 dimens.xml: @dimen/'+name);}));
ctx.subscriptions.push(vs.commands.registerCommand('androidLayoutEditor.wrapWithLayout',async()=>{const ed=vs.window.activeTextEditor;if(!ed)return;const sel=ed.selection;if(sel.isEmpty){vs.window.showWarningMessage('请先选择要包裹的内容');return;}const text=ed.document.getText(sel);const layout=await vs.window.showQuickPick(['LinearLayout','ConstraintLayout','FrameLayout','ScrollView'],{placeHolder:'选择包裹布局类型'});if(!layout)return;const wrapped='<'+layout+' xmlns:android="http://schemas.android.com/apk/res/android"\n    android:layout_width="match_parent"\n    android:layout_height="match_parent"\n    android:orientation="vertical">\n'+text+'\n</'+layout+'>';const edit=new vs.WorkspaceEdit();edit.replace(ed.document.uri,sel,wrapped);await vs.workspace.applyEdit(edit);vs.window.showInformationMessage('已用 '+layout+' 包裹');}));
ctx.subscriptions.push(vs.commands.registerCommand('androidLayoutEditor.unwrapLayout',async()=>{const ed=vs.window.activeTextEditor;if(!ed)return;const text=ed.document.getText();const m=text.match(/<(\w+)[^>]*>[\s\S]*?<\/\1>/);if(!m){vs.window.showWarningMessage('未找到可解开的布局');return;}const inner=m[0].replace(/^<\w+[^>]*>\s*/,'').replace(/\s*<\/\w+>\s*$/,'');const edit=new vs.WorkspaceEdit();edit.replace(ed.document.uri,new vs.Range(0,0,ed.document.lineCount,0),text.replace(m[0],inner));await vs.workspace.applyEdit(edit);vs.window.showInformationMessage('已解开外层布局');}));
...
const p=vs.window.createWebviewPanel('androidLayoutEditorPerf','Layout Performance',{viewColumn:vs.ViewColumn.Beside,preserveFocus:true},{enableScripts:true,retainContextWhenHidden:true});
const ed=vs.window.activeTextEditor;if(!ed||!ed.document.uri.path.endsWith('.xml')){p.webview.html='<h2>请先打开XML布局文件</h2>';return;}
const text=ed.document.getText();
const stats=analyzePerformance(text);
p.webview.html=buildPerfHtml(stats);
}));
...
function analyzePerformance(text){
const tags=text.match(/<\w+(\s|>)/g)||[];
const layouts=text.match(/<(LinearLayout|ConstraintLayout|FrameLayout|RelativeLayout|ScrollView|HorizontalScrollView)/g)||[];
const views=text.match(/<(TextView|Button|EditText|ImageView|ImageButton|CheckBox|RadioButton|Switch|ProgressBar|SeekBar|Spinner|RecyclerView|CardView|ViewStub|View)(\s|>)/g)||[];
let maxDepth=0,currentDepth=0;
const lines=text.split('\n');
for(const line of lines){const tl=line.trim();
if(tl.startsWith('<')&&!tl.startsWith('</')&&!tl.startsWith('<?')&&!tl.startsWith('<!--'))currentDepth++;
if(tl.startsWith('</'))currentDepth=Math.max(0,currentDepth-1);
if(currentDepth>maxDepth)maxDepth=currentDepth;
}
const measurePasses=maxDepth*2;
const overdraw=text.match(/android:background="#[^"]+"/g)||[];
return{totalTags:tags.length,layoutCount:layouts.length,viewCount:views.length,maxDepth:maxDepth,measurePasses:measurePasses,overdrawCount:overdraw.length,healthScore:Math.max(0,100-maxDepth*5-Math.max(0,tags.length-50))};
}
function buildPerfHtml(stats){
return `<!DOCTYPE html><html><head><style>body{font-family:system-ui,sans-serif;padding:20px;background:#1e1e2e;color:#cdd6f4;}h2{margin-top:0;} .metric{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #45475a;} .bar{height:8px;background:#313244;border-radius:4px;margin-top:4px;overflow:hidden;} .bar-fill{height:100%;background:#7c3aed;border-radius:4px;} .good{color:#22c55e;} .warn{color:#f59e0b;} .bad{color:#ef4444;} </style></head><body>
<h2>Layout Performance Analysis</h2>
<div class="metric"><span>Total Views</span><span>${stats.totalTags}</span></div>
<div class="metric"><span>Layouts</span><span>${stats.layoutCount}</span></div>
<div class="metric"><span>Leaf Views</span><span>${stats.viewCount}</span></div>
<div class="metric"><span>Max Depth</span><span class="${stats.maxDepth>8?'bad':stats.maxDepth>5?'warn':'good'}">${stats.maxDepth}</span></div>
<div class="metric"><span>Est. Measure Passes</span><span>${stats.measurePasses}</span></div>
<div class="metric"><span>Background Overdraw</span><span class="${stats.overdrawCount>3?'warn':'good'}">${stats.overdrawCount}</span></div>
<div class="metric"><span>Health Score</span><span class="${stats.healthScore>80?'good':stats.healthScore>50?'warn':'bad'}">${stats.healthScore}/100</span></div>
<div class="bar"><div class="bar-fill" style="width:${stats.healthScore}%"></div></div>
</body></html>`;
}
function deactivate(){}
